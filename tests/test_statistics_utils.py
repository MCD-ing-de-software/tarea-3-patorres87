...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        arrrespcorrecto = np.array([1.5, 2.5, 3.5])
        utils = StatisticsUtils()
        arrresp = utils.moving_average(arr, window=2)
        np.testing.assert_allclose(arrrespcorrecto, arrresp, rtol=1e-5, atol=0)
        self.assertEqual(arrrespcorrecto.shape,arrresp.shape)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        with self.assertRaises(ValueError):
            arrresp = utils.moving_average(arr, window=0)
        with self.assertRaises(ValueError):
            arrresp = utils.moving_average(arr, window=7)
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        zs = utils.zscore(arr)
        print(zs)
        self.assertAlmostEqual(zs.mean(), 0)
        self.assertAlmostEqual(zs.std(), 1)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        """
        arr = np.array([2, 4, 6])
        arrres = np.array([0.0, 0.5, 1.0])
        utils = StatisticsUtils()
        res = utils.min_max_scale(arr)
        self.assertAlmostEqual(res.min(), 0.0)
        self.assertAlmostEqual(res.max(), 1.0)
        np.testing.assert_allclose(res, arrres)
